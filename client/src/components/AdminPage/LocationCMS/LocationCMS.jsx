...
export const LocationCMS = ({ locationList, setLocationList }) => {
	...
	const handleUpdateLocation = async (updateRow, oldRow) => {
		await updateLocation(oldRow.lname, updateRow.lname, updateRow.lat, updateRow.long);
		setReloadLocation(!reloadLocation);
	};
	...
	return (
		<MaterialTable
			...
			options={{ pageSize: 10, actionsColumnIndex: -1, addRowPosition: 'first' }}
			editable={{
				onRowAdd: (newRow) => handleCreateLocation(newRow),
				onRowDelete: (selectedRow) => handleDeleteLocation(selectedRow),
				onRowUpdate: (updatedRow, oldRow) => handleUpdateLocation(updatedRow, oldRow),
			}}
		/>
	);
};
